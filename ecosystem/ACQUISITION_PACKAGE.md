...
| **WorthWyl Forge** | Creation, workspace, and operator product | Provides a practical environment for creating, directing, reviewing, and operating intelligent systems | Gives people the visible workspace through which the ecosystem is used and managed |
...
Cranium Ultra is the standalone integrated Core–Synapse product. It is not merely a bundle of two repositories. Its defining characteristic is that authority and capability are designed to operate together in one governed runtime.

Ultra can register capabilities, evaluate permissions, select providers, enforce policy, execute bounded workflows, produce receipts or provenance records, and support lifecycle operations such as registration, upgrade, quarantine, and revocation. A customer can adopt Ultra as a complete governed execution product without adopting the entire ecosystem.
...
The products compose into a **Cognitive Substrate Governance Ecosystem** through explicit contracts rather than hidden coupling. The ecosystem can be represented as follows:

```text
                 COGNITIVE SUBSTRATE GOVERNANCE ECOSYSTEM

                           CRANIUM AI
                 intelligence • dialogue • orchestration
                                   │
                          WORTHWYL FORGE
                 creation • workspace • operations
                                   │
       ┌──────────────────────────┼──────────────────────────┐
       │                          │                          │
 MIRACLE MEMORY            COGNITIVE TRACKER          CRANIUM ULTRA
 continuity                evaluation and evidence    governed runtime
       │                          │                          │
       └──────────────────────────┼──────────────────────────┘
                                  │
              shared identity • evidence • event contracts
```

Cranium Ultra contains the integrated Core–Synapse runtime:

```text
                           CRANIUM ULTRA
                    INTEGRATED CORE–SYNAPSE RUNTIME

                 ┌─────────────────┴─────────────────┐
                 │                                   │
           CRANIUM CORE                        CRANIUM SYNAPSE
           authority layer                     capability layer
                 │                                   │
       policy • permission • audit          models • tools • APIs
       approval • provenance • recovery     providers • connectors
                                             extensions • workflows
```

## A complete governed intelligence lifecycle
...
5. **Synapse selects the required capability.** The runtime identifies an appropriate model, provider, connector, tool, or workflow.
6. **Approval is requested when required.** The user sees the target, scope, data, consequences, cost, and irreversible effects before approval.
7. **The bounded action executes.** Synapse performs the operation through scoped credentials and resource limits.
...
The portfolio supports multiple acquisition or evaluation paths.

| Package | Contents | Buyer value |
|---|---|---|
...
## Required diligence presentation

The acquisition package should present the portfolio in layers so a reviewer can see both independent value and ecosystem value without confusing the two.

| Presentation layer | Question answered |
|---|---|
| Product card | What does this product do by itself? |
| Standalone deployment profile | Can it be deployed and operated independently? |
| Integration profile | Which contracts and events does it expose or consume? |
| Ecosystem map | How does it contribute to the combined system? |
| Evidence record | What tests, benchmarks, and artifacts support the claims? |
...
The acquisition opportunity is intentionally multidimensional. A buyer may acquire a focused product, adopt the integrated Core–Synapse runtime, combine continuity and evaluation capabilities, deploy the operator environment, or acquire the complete ecosystem. These paths are compatible because the portfolio is built around composability rather than forced centralization.

The central proposition is therefore simple:

> **Standalone products create independent value. Shared contracts create interoperability. Together, the products create a Cognitive Substrate Governance Ecosystem.**
