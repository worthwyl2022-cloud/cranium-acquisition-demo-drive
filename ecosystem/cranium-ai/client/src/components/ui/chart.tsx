    ...
    <div className={cn(
      ...
      <div className="grid gap-1.5">
        {payload.filter(item => item.type !== "none").map((item, index) => {
          ...
          return (
            <div key={item.dataKey ?? index} className={cn(
  ...
  return (
    <div className={cn(
      ...
      {payload.filter(item => item.type !== "none").map((item, index) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        return (
          <div key={item.value ?? item.dataKey ?? index} className="[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3">
