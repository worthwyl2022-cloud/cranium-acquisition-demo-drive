...
export type AttestationStatus = 'valid' | 'commitment_mismatch' | 'expired' | 'unavailable';
export type AssessmentDisposition = 'allow' | 'deny' | 'unavailable';
...
export interface AuthorityTransitionRequest {
  ...
  /** Optional protocol namespace for cross-domain isolation. */
  namespace?: string;
  /** Optional validated namespace carried by upstream evidence. */
  attestationNamespace?: string;
  /** Explicit evidence lifecycle status for protocol conformance. */
  attestationStatus?: AttestationStatus;
  /** Explicit semantic decision; this can restrict but never grant authority. */
...
export enum BoundaryViolation {
  ...
  SEMANTIC_POLICY_DENIAL = 'SEMANTIC_POLICY_DENIAL',
