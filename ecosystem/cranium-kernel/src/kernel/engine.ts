...
export class DefaultBoundaryValidator {
  validate(
    ...
    // Protocol evidence lifecycle and namespace checks are explicit Kernel
    // boundaries. Invalid evidence can restrict or deny, but never grant.
    checkedRules.push('RULE_00_EVIDENCE_LIFECYCLE_AND_NAMESPACE');
    if (request.attestationStatus === 'commitment_mismatch') {
      violations.push(BoundaryViolation.INVALID_EVIDENCE_STATUS);
    } else if (request.attestationStatus === 'expired') {
      violations.push(BoundaryViolation.STALE_EVIDENCE);
    } else if (request.attestationStatus === 'unavailable') {
      violations.push(BoundaryViolation.EVIDENCE_UNAVAILABLE);
    }
    if (request.assessmentDisposition === 'deny') {
      violations.push(BoundaryViolation.SEMANTIC_POLICY_DENIAL);
    } else if (request.assessmentDisposition === 'unavailable') {
      violations.push(BoundaryViolation.EVIDENCE_UNAVAILABLE);
    }
    if (
      request.namespace &&
      request.attestationNamespace &&
      request.namespace !== request.attestationNamespace
    ) {
      violations.push(BoundaryViolation.NAMESPACE_CONFLICT);
    }
...
export class DefaultAuthorityTransitionEngine {
  ...
  evaluate(
    ...
    // Constitutional state is a prerequisite for authority evaluation. A
    // caller may not construct a reduced or tampered state and still obtain a
    // canonical transition from the engine.
...
export class KernelStateReducer {
  static reduce(
    ...
    assertConstitutionalState(state);
    assertConstitutionalTransition(transition);
